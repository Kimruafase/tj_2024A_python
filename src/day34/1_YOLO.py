...
def make_img_label():
  ...
  for n_h in range(num_shape):
    ...
    y = int(y_rate * hight_size/cell_num + col * hight_size/cell_num)
...
for e in range(epoch_num):
  ...
  with tf.GradientTape() as tape:
    pred = yolo_model(img) #이미지를 구분한 셀을 탐험
    for i in range(3):
      for j in range(3):
...
#해당 셀에 객체가 없을 경우 객체가 없을 확률만 Loss로 계산
    ...
    loss_p_no = tf.reduce_mean(loss_p_no_list) #각 Loss를 비중을 곱해 더해 최종 Loss를 계산
...
#Loss에 대한 Grad를 구하고, 각 파라미터를 업데이트
  ...
  opt.apply_gradients(zip(grad, vars)) #100번마다 동영상에 이미지를 기록
  if(e%100==0):
